...
export function shiftDailyOutOfQuietHours(
  hour: number,
  minute: number,
): { hour: number; minute: number } {
  const { quietHoursEnabled, quietStartMinutes, quietEndMinutes } =
    useNotificationsStore.getState();
  if (!quietHoursEnabled) return { hour, minute };
  if (!isInQuietWindow(hour * 60 + minute, quietStartMinutes, quietEndMinutes))
    return { hour, minute };
